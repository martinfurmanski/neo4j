 ...
 */
public class PhysicalRaftLog extends LifecycleAdapter implements RaftLog
{
...
//            termCache.populate( state.appendIndex, entry.term() );
        ...
        {
            switchSegment( state.appendIndex, state.appendIndex, state.currentTerm );
        }
    ...
    public synchronized void truncate( long fromIndex ) throws IOException, RaftLogCompactedException
    {
        ...
        switchSegment( state.appendIndex, newAppendIndex, newTerm );

        state.appendIndex = newAppendIndex;
        state.currentTerm = newTerm;
    }

    private void switchSegment( long prevFileLastIndex, long prevIndex, long prevTerm ) throws IOException
    {
        state.segments.last().closeWriter();
        state.segments.createNext( prevFileLastIndex, prevIndex, prevTerm );
    }
    ...
    public synchronized long skip( long newIndex, long newTerm ) throws IOException
    {
        ...
        {
            switchSegment( state.appendIndex, newIndex, newTerm );
