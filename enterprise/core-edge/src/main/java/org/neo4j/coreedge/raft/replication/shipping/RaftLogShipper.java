...
public class RaftLogShipper<MEMBER>
{
    ...
    public synchronized void onNewEntry( long prevLogIndex, long prevLogTerm, RaftLogEntry[] newLogEntries, LeaderContext leaderContext )
    {
        switch ( mode )
        {
        case PIPELINE:
            while( lastSentIndex <= prevLogIndex )
            {
                ...
                {
                    sendNewEntry( prevLogIndex, prevLogTerm, newLogEntries, leaderContext ); // all sending functions update lastSentIndex
                }
                else
                {
    ...
    private void sendNewEntry( long prevLogIndex, long prevLogTerm, RaftLogEntry[] newEntries, LeaderContext leaderContext )
    {
        scheduleTimeout( retryTimeMillis );
