...
public class SimpleDiscoveryServiceFactory
{
    public static CoreDiscoveryService coreDiscoveryService( Config config )
    {
        ...
        fixedDiscoveryAddresses.addAll( initialMembers );

        // TODO: Make user-configurable. Tests should use short interval.
        int refreshIntervalMillis = 1000;
        int timeoutIntervalMillis = 5000;

        AdvertisedSocketAddress myDiscoveryAddress = new AdvertisedSocketAddress( listenAddress.toString() );
        ...
        return new SimpleDiscoveryService( myself, listenAddress, myDiscoveryAddress, clusterUUID, fixedDiscoveryAddresses, logProvider, jobScheduler,
                refreshIntervalMillis, timeoutIntervalMillis, initialMembers.size() );
    }

    public EdgeDiscoveryService edgeDiscoveryService()
    {
