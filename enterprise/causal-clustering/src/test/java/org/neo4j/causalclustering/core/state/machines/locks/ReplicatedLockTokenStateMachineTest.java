...
public class ReplicatedLockTokenStateMachineTest
{
    ...
    public void shouldIssueNextLockTokenCandidateId() throws Exception
    {
        ...
        // when
        stateMachine.applyCommand( new ReplicatedLockToken( member( 0 ), firstCandidateId ), 0, r -> {} );
    ...
    public void shouldKeepTrackOfCurrentLockTokenId() throws Exception
    {
        ...
        // when
        stateMachine.applyCommand( new ReplicatedLockToken( member( 0 ), firstCandidateId ), 1, r -> {} );

        // then
        assertEquals( firstCandidateId, stateMachine.currentToken().id() );

        // when
        stateMachine.applyCommand( new ReplicatedLockToken( member( 0 ), firstCandidateId + 1 ), 2, r -> {} );
    ...
    public void shouldKeepTrackOfLockTokenOwner() throws Exception
    {
        ...
        // when
        stateMachine.applyCommand( new ReplicatedLockToken( member( 1 ), firstCandidateId + 1 ), 2, r -> {} );
    ...
    public void shouldAcceptOnlyFirstRequestWithSameId() throws Exception
    {
        ...
        // when
        stateMachine.applyCommand( new ReplicatedLockToken( member( 0 ), firstCandidateId ), 1, r -> {} );
        stateMachine.applyCommand( new ReplicatedLockToken( member( 1 ), firstCandidateId ), 2, r -> {} );
        ...
        // when
        stateMachine.applyCommand( new ReplicatedLockToken( member( 1 ), firstCandidateId + 1 ), 3, r -> {} );
        stateMachine.applyCommand( new ReplicatedLockToken( member( 0 ), firstCandidateId + 1 ), 4, r -> {} );
    ...
    public void shouldOnlyAcceptNextImmediateId() throws Exception
    {
        ...
        // when
        stateMachine.applyCommand( new ReplicatedLockToken( member( 0 ), firstCandidateId + 1 ), 1, r -> {} ); // not accepted

        // then
        assertEquals( stateMachine.currentToken().id(), LockToken.INVALID_LOCK_TOKEN_ID );

        // when
        stateMachine.applyCommand( new ReplicatedLockToken( member( 0 ), firstCandidateId ), 2, r -> {} ); // accepted

        // then
        assertEquals( stateMachine.currentToken().id(), firstCandidateId );

        // when
        stateMachine.applyCommand( new ReplicatedLockToken( member( 0 ), firstCandidateId + 1 ), 3, r -> {} ); // accepted

        // then
        assertEquals( stateMachine.currentToken().id(), firstCandidateId + 1 );

        // when
        stateMachine.applyCommand( new ReplicatedLockToken( member( 0 ), firstCandidateId ), 4, r -> {} ); // not accepted

        // then
        assertEquals( stateMachine.currentToken().id(), firstCandidateId + 1 );

        // when
        stateMachine.applyCommand( new ReplicatedLockToken( member( 0 ), firstCandidateId + 3 ), 5, r -> {} ); // not accepted
    ...
    public void shouldPersistAndRecoverState() throws Exception
    {
        ...
        try ( Lifespan lifespan = new Lifespan( storage ) )
        {
            ...
            // when
            candidateId = 0;
            stateMachine.applyCommand( new ReplicatedLockToken( memberA, candidateId ), 0, r -> {} );
            candidateId = 1;
            stateMachine.applyCommand( new ReplicatedLockToken( memberB, candidateId ), 1, r -> {} );
    ...
    public void shouldBeIdempotent() throws Exception
    {
        ...
        try ( Lifespan lifespan = new Lifespan( storage ) )
        {
            ...
            stateMachine.applyCommand( new ReplicatedLockToken( memberA, 0 ), 3, r ->
            {
            } );

            // when
            stateMachine.applyCommand( new ReplicatedLockToken( memberB, 1 ), 2, r ->
            {
            } );
    ...
    public void shouldSetInitialPendingRequestToInitialState() throws Exception
    {
        ...
        ReplicatedLockTokenState initialState = new ReplicatedLockTokenState( 123, new ReplicatedLockToken( initialHoldingCoreMember, 3 ) );
