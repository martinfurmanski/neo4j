...
public class CatchupPollingProcess extends LifecycleAdapter
{
    ...
    public CatchupPollingProcess( LogProvider logProvider, FileSystemAbstraction fs, LocalDatabase localDatabase,
            ...
            CopiedStoreRecovery copiedStoreRecovery, DatabaseHealth dbHealth )
    {
        this.fs = fs;
    ...
    private synchronized void resetApplier() throws Exception
    {
        applier.emptyQueueAndResetLastQueuedTxId(); // TODO: each round must make sure to finalize its work... get rid of this?
    }

    private synchronized void panic( Throwable e )
    {
        log.error( "Unexpected issue in catchup process. No more TX pull requests will be scheduled.", e );
        dbHealth.panic( e );
        state = PANIC;
    }
