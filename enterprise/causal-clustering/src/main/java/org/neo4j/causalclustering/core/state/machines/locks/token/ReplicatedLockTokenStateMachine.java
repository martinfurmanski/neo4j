...
/**
 * Listens for {@link ReplicatedLockToken}. Keeps track of the current holder of the replicated token,
 * which is identified by a monotonically increasing id, and an owning member.
 */
public class ReplicatedLockTokenStateMachine implements StateMachine<ReplicatedLockToken>
{
    ...
    @Override
    public synchronized void applyCommand( ReplicatedLockToken tokenRequest, long commandIndex,
     ...
     */
    public synchronized ReplicatedLockToken currentToken()
    {
        return state().get();
    }
}
