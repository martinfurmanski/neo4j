...
public class CoreClientResponseAdaptor<T> implements CoreClientResponseCallback<T>
{
    @Override
    public void onFileHeader( CompletableFuture<T> signal, FileHeader response )
    {
        signal.completeExceptionally( new CoreProtocolViolationException( "Unexpected response: %s", response ) );
    }
    ...
    @Override
    public void onLockResponse( CompletableFuture<T> signal, LockResponse response )
    {
        signal.completeExceptionally( new CoreProtocolViolationException( "Unexpected response: %s", response ) );
    }
}
